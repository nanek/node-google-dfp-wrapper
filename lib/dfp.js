...
Dfp.prototype.getLineItems = function(conditions) {
  ...
  var query = _makeQuery(conditions, fields);
 ...
 * @return {String}            The system assigned id of the passed in key.
 */
Dfp.prototype.lookupCriteriaKey = function(conditions) {
  return criteriaKeyStore.getAsync(conditions.name)
    .bind(this)
    .catch(function() {
      // not found in store, look up instead
      return this.getCriteriaKey(conditions)
        .tap(function(id) {
          return criteriaKeyStore.putAsync(conditions.name, id)
            .catch(function(e) {
 ...
 * @return {Array}        An array of ids of the matching criteria values.
 */
 ...
 * @return {String}            The system assigned id of the passed in key.
 */
Dfp.prototype.lookupCriteriaValues = function(conditions) {
  var lookupKey = conditions.keyId + ':' + conditions.name;
  ...
  return criteriaValueStore.getAsync(lookupKey)
    ...
    .catch(function() {
      ...
      return this.dfpUser.executeAPIFunction(service, method, query)
        .then(extractResults)
        .map(function(result) {
          var keyForResult = conditions.keyId + ':' + result.name;
          return criteriaValueStore.putAsync(keyForResult, result.id);
        })
        .catch(function(e) {
...
Dfp.prototype.getLabel = function(conditions) {
  ...
  return labelStore.getAsync(query)
    .catch(function() {
      return ctx.dfpUser.executeAPIFunction(service, method, query)
