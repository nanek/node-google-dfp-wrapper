  ...
  return criteriaValueStore.getAsync(storeKey)
    .catch(function() {
      ...
      return ctx.dfpUser.executeAPIFunction(service, method, query)
        .then(function(response) {
          return Bluebird.resolve(response.results)
            .map(function(results) {
              return criteriaValueStore.putAsync(storeKey, results.id);
            })
            .catch(function(storePutErr) {
              console.log(
                'locally storing criteria value failed',
                storePutErr.stack
              );
            });
        })
        ...
        .catch(function(storeGetErr) {
          console.log('getting criteria value failed', storeGetErr.stack);
        });
    })
    .then(function(id) {
      return [id];
